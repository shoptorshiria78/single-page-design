      ...
      <div className="max-w-[1000px] h-[500px] mx-auto px-44 mt-10 relative hidden lg:grid lg:grid-cols-3 lg:gap-3  ">

        {
          sliderData.slice(firstIndex, lastIndex).map(item =>
            <div key={item.id}>
              <div className="w-[200px] bg-[#3B3E47] shadow-2xl duration-500 ">
                ...
                <div className="text-center p-2 text-xs  my-4 relative">
                  <div className="w-3 h-6 rounded-tr-full  rounded-br-full bg-[#18282A] absolute top-2 -left-0"></div>
                  <hr className=" border-dashed border-[1px] border-gray-400  my-3" />
                  <div className="w-3 h-6 rounded-tl-full rounded-bl-full bg-[#18282A] absolute top-2 -right-0 "></div>
                  <div className="px-2">
                    <p className="text-[14px] text-white my-2 font-semibold">{item.title}</p>
                    <p className=" text-[10px] text-white font-medium my-2 "><span className="mx-2">{item.date}</span>|<span className="mx-2">{item.weekDay}</span>|<span className="mx-2">{item.time}</span> PM</p>
                    <p className="mx-2 text-white text-[10px]">{item.address}</p>
      ...
      <div className="max-w-[1200px] h-[500px] mx-auto  mt-10 relative grid grid-cols-1 lg:hidden ">

        {
          sliderData.slice(startIndex, position).map(item =>
            <div key={item.id}>
              <div className=" w-[257px] bg-[#3B3E47] shadow-2xl duration-500 mx-auto ">
                ...
                <div className="text-center p-2 text-xs  my-4 relative">
                  ...
                  <div className="w-3 h-6 rounded-tl-full rounded-bl-full bg-[#18282A] absolute top-2 -right-0 "></div>
                  <div className="px-2">
                    <p className="text-lg text-white my-2 font-semibold">{item.title}</p>
                    <p className=" text-xs text-white font-medium my-2 "><span className="mx-2">{item.date}</span>|<span className="mx-2">{item.weekDay}</span>|<span className="mx-2">{item.time}</span> PM</p>
                    <p className="mx-3 text-white">{item.address}</p>
                    <button className="bg-black text-white w-full py-2 mt-3 ">{item.collection}</button>
                  </div>
                </div>
              </div>
...
export default CollectionCardDark;
